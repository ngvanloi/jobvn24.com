...
$q = "nf_scrap as ns right join (nf_member as nm right join nf_resume as nr on nm.`no`=nr.`mno`) on ns.`pno`=nr.`no` where ns.`code`='resume' and ns.`mno`=".intval($member['no'])." ".$_where;
...
$scrap_query = $db->_query("select *, ns.`no` as ns_no, ns.`mno` as ns_mno, ns.`pno` as nr_no from ".$q.$order." limit ".$paging['start'].", ".$_arr['num']);
?>
<div class="m_title">
	<button class="back" onclick="history.back()"><i class="axi axi-keyboard-arrow-left"></i></button>스크랩한 인재정보<button class="forward" onclick="history.forward()"><i class="axi axi-keyboard-arrow-right"></i></button>
</div>
