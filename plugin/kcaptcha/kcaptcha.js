$(function(){
    ...
    $(document).on( "click", "#captcha_reload", function(){
        $.ajax({
            type: 'POST',
            url: root+'/plugin/kcaptcha/kcaptcha_session.php',
            cache: false,
            async: false,
            success: function(text) {
                $('#captcha_img').attr('src', root+'/plugin/kcaptcha/kcaptcha_image.php?t=' + (new Date).getTime());
            }
        });

        $.ajax({
            type: 'POST',
            url: root+'/plugin/kcaptcha/kcaptcha_mp3.php',
            cache: false,
            async: false,
...
function chk_captcha()
{
    ...
    $.ajax({
        type: 'POST',
        url: root+'/plugin/kcaptcha/kcaptcha_result.php',
        data: {
