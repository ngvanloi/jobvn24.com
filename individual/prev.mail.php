...
var reply_message = function(el, no) {
	...
	$.post(root+"/include/regist.php", "mode=get_message_info&code=<?php echo $_GET['code'];?>&no="+no, function(data){
		data = $.parseJSON(data);
...
var click_message = function(el, no) {
	$.post(root+"/include/regist.php", "mode=click_message&code=<?php echo $_GET['code'];?>&no="+no, function(data){
		data = $.parseJSON(data);
		if(data.msg) alert(data.msg);
		if(data.move) location.href = data.move;
		if(data.js) eval(data.js);
			...
			<section class="mail_page">
				...
				<table class="style3">
					...
					<tr>
						...
						<td><?php echo $_GET['code']=='send' ? $nf_util->get_text($row['pmb_nick']) : $nf_util->get_text($row['mb_nick']);?></td>
					...
					<tr style="display:none;">
						<td colspan="5" class="tal mail_con">
							...
							<ul class="fr">
								<li><a href="#none" onClick="reply_message(this, '<?php echo $row['no'];?>')">답장하기</a></li>
								<li><a href="">닫기</a></li>
							</ul>
						</td>
					</tr>
